...
const CronBox = ({ title, value }: { title: string; value: string }) => (
    <div css={tw`rounded p-3`} style={{ background: 'var(--bh-surface2)', border: '1px solid var(--bh-border)', transition: 'background 0.3s, border-color 0.3s' }}>
        <p css={tw`text-sm`} style={{ color: 'var(--bh-text)', opacity: 0.55 }}>{title}</p>
        <p css={tw`text-xl font-medium`} style={{ color: 'var(--bh-text)', transition: 'color 0.3s' }}>{value}</p>
    </div>
);
    ...
    return (
        <PageContentBlock title={'Schedules'}>
            ...
            {!schedule || isLoading ? (
                ...
                <>
                    <BeforeEdit />
                    <ScheduleCronRow cron={schedule.cron} css={tw`sm:hidden rounded mb-4 p-3`} style={{ background: 'var(--bh-surface2)', border: '1px solid var(--bh-border)' }} />
                    <div css={tw`rounded shadow`}>
                        <div
                            css={tw`sm:flex items-center p-3 sm:p-6 rounded-t`}
                            style={{ background: 'var(--bh-surface)', borderBottom: '1px solid var(--bh-border)', transition: 'background 0.3s, border-color 0.3s' }}
                        >
                            <div css={tw`flex-1`}>
                                <h3 css={tw`flex items-center text-neutral-100 text-2xl`}>
                                    ...
                                    {schedule.isProcessing ? (
                                        <span
                                            ...
                                            style={{ background: 'var(--bh-surface2)', color: 'var(--bh-text)', border: '1px solid var(--bh-border)' }}
                                        >
                        ...
                        <div css={tw`rounded-b`} style={{ background: 'var(--bh-surface)', border: '1px solid var(--bh-border)', borderTop: 'none', transition: 'background 0.3s, border-color 0.3s' }}>
